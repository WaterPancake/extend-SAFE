...
- [4. Supporting findings](#4-supporting-findings)
  ...
  - [4.2 Latent geometry is organized strongly by
    task](#42-latent-geometry-is-organized-strongly-by-task)
...
4.  SAFE’s functional conformal procedure does not produce an operating
    ...
    held-out tasks.
5.  The tested layer-fusion methods do not reliably improve
    held-out-task AUC.
...
4.  That functional conformal prediction is generally invalid. The
    ...
    reproduction.
5.  That additional layers can never help; only that the tested PCA/PLS
    fusion methods show no reliable held-out gain.
